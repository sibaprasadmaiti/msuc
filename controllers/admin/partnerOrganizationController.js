    ...
    jwt.verify(token, SECRET, async function(err, decoded) {
        if (err) {
            ...
            if (sessionStoreId == null) {
                ...
                let brandTitles = await models.partnerOrganization.findAll({ attributes: ['title'] });
                let brandList
                let listCount

                if(title!='' || status !=''){
                    brandList = await models.partnerOrganization.findAll({ where: whereCondition , order: [[column, order]], limit:pageSize, offset:(page-1)*pageSize });

                    listCount = await models.partnerOrganization.count({ where: whereCondition});
                }else{
                    brandList = await models.partnerOrganization.findAll({ where: {
                        [Op.or]: [
                    ...
                    listCount = await models.partnerOrganization.count({ where: {
                        [Op.or]: [
                            {title: {[Op.like]:`%${search}%`}},
                            {image: {[Op.like]:`%${search}%`}},
                            {shortDescriptions: {[Op.like]:`%${search}%`}},
                ...
                if (brandList) {
                    return res.render('admin/partnerOrganization/list', {
                        title: 'Partner Organization List',
                        arrData: brandList,
                        arrStore: storeList,
                ...
                if (userPermission == false) {
                    ...
                    let brandTitles = await models.partnerOrganization.findAll({ attributes: ['title'],storeId: sessionStoreId });
                    let brandList
                    let listCount
                    ...
                    if(title!='' || status !=''){
                        ...
                        brandList = await models.partnerOrganization.findAll({ where: {storeId: sessionStoreId,
                            [Op.or]: [
                                {title: {[Op.like]:`%${search}%`}},
                                {image: {[Op.like]:`%${search}%`}},
                                {shortDescriptions: {[Op.like]:`%${search}%`}},
                        ...
                        listCount = await models.partnerOrganization.count({ where: {storeId: sessionStoreId,
                            [Op.or]: [
                                {title: {[Op.like]:`%${search}%`}},
                                {image: {[Op.like]:`%${search}%`}},
                                {shortDescriptions: {[Op.like]:`%${search}%`}},
    ...
    jwt.verify(token, SECRET, async function(err, decoded) {
        if (err) {
            ...
            if (req.session.permissions.length == 0 && req.session.role == '' && sessionStoreId == null) {
                ...
                if (id){
                    var storeIdChecking = await models.partnerOrganization.findOne({ attributes: ['storeId'], where: { id: id } });
            ...
            if (userPermission == false) {
                ...
                if (!id) {
                    return res.render('admin/partnerOrganization/addedit', {
                    ...
                    var brandDetails = models.partnerOrganization.findOne({
...
exports.addOrUpdate = function(req, res) {
    ...
    jwt.verify(token, SECRET, async function(err, decoded) {
        if (err) {
            ...
            try {
                if(!id){
                    const partnerOrganization = await models.partnerOrganization.create({
                        title:title,
                        type:type,
                        storeId:storeId,
                        status:status,
                    ...
                    if (
                        ...
                        const dir = `public/admin/partnerOrganization/`;
                        if (!fs.existsSync(dir)) {
                          fs.mkdirSync(dir);
                        }
                        const fileTitle = Date.now();
                        const path =`public/admin/partnerOrganization/${fileTitle}.${imageExt}`;
                        ...
                        await models.partnerOrganization.update(
                          { image: normalfile },
                          { where: { id: partnerOrganization.id } }
                        );
                      }
                      req.flash('info','Successfully Partner Organization created');
                      return res.redirect('/admin/partnerOrganization/list/1');
                }else{
                    const partnerOrganization = await models.partnerOrganization.update({
                        title:title,
                        type:type,
                        storeId:storeId,
                        status:status,
                    ...
                    if (
                      ...
                      req.flash('info','Successfully Partner Organization updated');
                      return res.redirect('/admin/partnerOrganization/list/1');
                }   
            } catch (error) {
        ...
        if(err){
            ...
            if (userPermission == false) {
                ...
                if(products.length>0){
                    ...
                    models.partnerOrganization.destroy({
                        where: whereCondition
                    }).then(function (value) {
                        if (value) {
